...
public static class LogicFunctionMinimizer
{
        ...
        MinimizeOptions options)
    {
        ...
        var minimizedPlaText = EspressoNative.MinimizePla(
            plaText,
            ToEspressoMode(options, logicFunction.OutputNames.Length));
    ...
    private static EspressoMinimizeMode ToEspressoMode(MinimizeOptions options, int outputCount)
    {
        ...
        {
            return options.MinimizeOutputsIndependently || outputCount == 1
                ? EspressoMinimizeMode.ExactIndependentOutput
                : EspressoMinimizeMode.ExactJoint;
        }

        return options.MinimizeOutputsIndependently || outputCount == 1
            ? EspressoMinimizeMode.FastIndependentOutput
            : EspressoMinimizeMode.FastJoint;
    }
