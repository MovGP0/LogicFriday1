...
public static class Complement
{
    ...
    private static BitVectorFamily ComputeComplement(CubeData cube, CubeList T, SplitStack stack, int depth)
    {
                ...
                if (!cscHandled)
                {
                    ...
                    else
                    {
                        ...
                        else if (analysis.VarsUnate == analysis.VarsActive)
                        {
                            ...
                            // Compute
                            {
                                for (int si = 0; si < ucA.Count; si++) BitVectorOps.SetSortKey(ucA.GetSet(si), BitVectorOps.PopCount(ucA.GetSpan(si)));
                                ucA = UnateComplement.ComplementRecursive(ucA);
                                if (ucA.Count > 0)
                                {
                                    for (int i = 0; i < ucA.Count; i++) BitVectorOps.SetSortKey(ucA.GetSet(i), BitVectorOps.PopCount(ucA.GetSpan(i)));
                                    ...
                                    for (int i = 0; i < ucA.Count; i++)
                                    {
                                        ...
                                        for (int j = i + 1; j < ucA.Count; j++)
                                        {
                                            ...
                                            if (BitVectorOps.PopCount(ucA.GetSpan(ucOrder[j])) < iPop) continue;
                                            if (BitVectorOps.IsSubsetOf(ucA.GetSpan(ucOrder[i]), ucA.GetSpan(ucOrder[j]))) ucKeep[ucOrder[j]] = false;
                                        }
                                    }
                                    int ucCnt = 0;
                                    for (int i = 0; i < ucA.Count; i++)
                                    ...
                                    for (int ncol = 0; ncol < nunate; ncol++)
                                        if (BitVectorOps.Contains(ucsp, ncol))
                                            for (int i = cube.FirstPart![unate[ncol]]; i <= cube.LastPart![unate[ncol]]; i++)
                                                if (analysis.PartZeros[i] == 0) BitVectorOps.Remove(ucspB, i);
                                }
        ...
        // --- inlined MergeComplements ---
        {
                ...
                while (li < L1.Length && ri < R1.Length)
                    switch (CubeDistance.Distance1Order(cube, L1[li], R1[ri]))
                    {
                        ...
                        default:
                            ...
                            BitVectorOps.Or(sL, sL, R1[ri].AsSpan());                    ri++;
                            break;
                    }
            }
            switch(lifting)
            {
                case UseComplLiftOnset:
                {
                    ...
                    BitVectorFamily Tcover;
                    {
                        ...
                        for (int mci = 0; mci < T.Count; mci++) BitVectorOps.Or(Tcover.GetSpan(mci), T.GetSpan(mci), mcCofSpan);
            ...
            for (int i = 0; i < Tr.Count; i++)
                if (BitVectorOps.HasFlag(Tr.GetSet(i), CubeFlags.Active)) BitVectorFamily.Add(Tbar, Tr.GetSet(i));
        }
    ...
    private static void LiftComplement(CubeData cube, BitVector[] A1, BitVector[] B1, BitVector bcube, int var)
    {
        ...
        {
            BitVector a = A1[ai];
            if (!BitVectorOps.HasFlag(a, CubeFlags.Active)) continue;
            ...
            {
                if (BitVectorOps.PopCount(B1[bi].AsSpan()) < liftPop) continue;
                if (!BitVectorOps.IsSubsetOf(slift, B1[bi].AsSpan())) continue;
    ...
    private static void LiftComplementOnset(CubeData cube, BitVector[] A1, BitVectorFamily T, BitVector bcube, int var)
    {
        ...
        for (int ai = 0; ai < A1.Length; ai++)
        {
            ...
            for (int ti = 0; ti < T.Count; ti++)
                if (CubeDistance.AreDistance0(cube, T.GetSpan(ti), slift)) { canLift = false; break; }
            if (canLift)
            {
