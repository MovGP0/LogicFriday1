...
public static class Irredundant
{
    ...
    public static void MarkIrredundant(CubeData cube, BitVectorFamily F, BitVectorFamily D, SplitStack stack)
    {
        ...
        BitVectorFamily E, Rp;
        {
            ...
            for (int si = 0; si < F.Count; si++)
            {
                ...
                if (IsCubeCovered(cube, FD, p, stack)) R = BitVectorFamily.Add(R, p);
                else E = BitVectorFamily.Add(E, p);
            }
            FD.ReturnCubes();
            ...
            for (int si = 0; si < R.Count; si++)
            {
                ...
                if (!IsCubeCovered(cube, ED, p, stack)) Rp = BitVectorFamily.Add(Rp, p);
            }
            ED.ReturnCubes();
        }
        ...
        SparseMatrix coverTable;
        {
            ...
            for (int j = 0; j < Rp.Count; j++)
            {
                ...
                FunctionalTautology(cube, cof, coverTable, (int)BitVectorOps.GetSortKey(p), stack, 0);
                cof.ReturnCubes();
        ...
        {
            BitVector p1 = F.GetSet((int)BitVectorOps.GetSortKey(E.GetSet(i)));
        ...
        // --- inlined TautologySpecialCases ---
        {
            ...
            while (true)
            {
                ...
                for (int ti = 0; ti < T.Count; ti++)
                {
                    ...
                    if (firstPass && CubeDistance.IsFullCoverage(cube, sp, scof)) { tscResult = TautTrue; break; }
                    BitVectorOps.Or(sceil, sceil, sp);
                }
                if (tscResult != TautMaybe) break;
                firstPass = false;
                if (!BitVectorOps.AreEqual(sceil, sFull)) { tscResult = TautFalse; break; }
        ...
        // --- inlined FunctionalTautologySpecialCases ---
        {
            ...
            while (true)
            {
                for (int fi = 0; fi < T.Count; fi++)
                    if (!BitVectorOps.HasFlag(T[fi], CubeFlags.Redund) && CubeDistance.IsFullCoverage(cube, T.GetSpan(fi), scof)) { ftscResult = TautTrue; break; }
                ...
                if (summary.VarsUnate == summary.VarsActive)
                {
                    ...
                    for (int fi = 0; fi < T.Count; fi++)
                        if (BitVectorOps.HasFlag(T[fi], CubeFlags.Redund) && CubeDistance.IsFullCoverage(cube, T.GetSpan(fi), scof))
                            SparseMatrix.Insert(table, rownum, (int)BitVectorOps.GetSortKey(T[fi]));
                    ftscResult = TautTrue;
                    break;
                }
                if (summary.VarsUnate != 0)
    ...
    private static CubeList FilterUnate(CubeData cube, CubeList T, VariableAnalysis analysis, BitVector ceil, BitVector temp)
    {
        ...
        for (int v = 0; v < cube.NumVars; v++)
            if (analysis.IsUnate[v]) BitVectorOps.Or(sceil, sceil, cube.VarMask[v].AsSpan());
        ...
        for (int i = 0; i < T.Count; i++)
        {
            ...
            if (BitVectorOps.IsSubsetOf(sceil, stemp)) filtered[fc++] = T[i];
        }
