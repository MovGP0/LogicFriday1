...
public static class MinimumCoverSolver
{
    public static SparseEntry Solve(SparseMatrix A)
    {
        ...
        {
            if (!prow.Refs.Overlaps(sol.Refs))
            {
    ...
    private static CoverSolution? SolveRecursive(SparseMatrix A, CoverSolution select, int lb, int bound)
    {
        bool essenDone = false;
        {
            ...
            do
            {
                ...
                foreach (var prow in A.Rows)
                {
                    if (prow.Refs.Count == 1)
                    {
                        essen.Add(prow.Refs.Min);
                    }
                }
...

        {
            ...
            if (reduceIt)
            {
                ...
                for (int ri = 0; ri < c2RefCnt; ri++)
                {
                    ...
                    {
                        foreach (int col in saveSec.Refs)
                        {
                            SparseMatrix.Insert(A, c2RefBuf[ri], col);
                        }
                    }
                }
                ...
                if (gBest != null)
                {
                    if (saveSec.Refs.Overlaps(gBest.Entry.Refs))
                    {
                        gBest.Add(c2Key);
                    }
                    else
        ...
        var indep = new CoverSolution();
        {
            ...
            foreach (var prow in A.Rows)
            {
                ...
                {
                    totalCap += A.Cols[col]!.Refs.Count;
                }
                ...
                for (int i = 0; i < bc; i++)
                {
                    if (unique == 0 || bimBuf[unique - 1] != bimBuf[i])
                    {
                        bimBuf[unique++] = bimBuf[i];
                    }
                }
                ...
                {
                    if (!B.Cols.TryGetValue(row, out var pcol))
                    {
                        pcol = new SparseEntry { Key = row };
                        B.Cols.Set(row, pcol);
                    }
            ...
            while (B.Rows.Count > 0)
            {
                ...
                {
                    if (bestRow == null || bprow.Refs.Count < bestRow.Refs.Count)
                    {
                        bestRow = bprow;
                    }
                }
        ...
        if (A.NRows > 0)
        {
            ...
            if (!Dominance.Visit(A, firstRow!, visitedRows, visitedCols, A.Rows, A.Cols))
            {
                foreach (var prow in A.Rows)
                {
                    ...
                    foreach (int col in prow.Refs)
                    {
                        if (!target.Cols.TryGetValue(col, out var pcol))
                        {
                            pcol = new SparseEntry { Key = col };
                            target.Cols.Set(col, pcol);
                        }
        ...
        if (blockResult != 0)
        {
            ...
            foreach (int col in leftBest.Entry.Refs)
            {
                select.Add(col);
            }
        ...
        int branchCol;
        {
            ...
            {
                totalCap += A.Rows[indepRow]!.Refs.Count;
            }
            ...
            for (int i = 0; i < bc; i++)
            {
                ...
                double w = 0.0;
                foreach (int row in sbpcol.Refs)
                {
                    var rEntry = A.Rows[row]!;
                    w += 1.0 / (rEntry.Refs.Count - 1.0);
                }
