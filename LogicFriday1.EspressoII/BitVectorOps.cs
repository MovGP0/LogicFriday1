...
public static class BitVectorOps
{
    ...
    public static void AddFlag(BitVector set, CubeFlags flag)
    {
        if (!set.IsNull)
        {
            set.Meta |= (uint)flag;
    ...
    public static void ClearFlag(BitVector set, CubeFlags flag)
    {
        if (!set.IsNull)
        {
            set.Meta &= ~(uint)flag;
    ...
    public static void SetSortKey(BitVector set, int size)
    {
        if (!set.IsNull)
        {
            set.Meta = (set.Meta & 0xFFFFu) | ((uint)size << 16);
        }
    }
