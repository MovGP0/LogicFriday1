    ...
    public static int Read(TextReader fp, bool needsOffset, out PlaData? plaReturn)
    {
        ...
        while ((line = reader.ReadLine()) != null)
        {
            if (line[0] == '.')
            {
                ...
                if (spaceIdx >= 0)
                {
                    reader.ResetToLine(line[(spaceIdx + 1)..]);
                }

                switch (directive)
                {
                    case "i":
                        if (!cube.FullSet.IsNull)
                        {
                            break;
                        }
                        ...
                        if (n < 0)
                        {
                            throw new InvalidOperationException("num_binary_vars cannot be negative");
                        }
                    ...
                    case "o":
                        ...
                        break;
                    case "mv":
                        if (!cube.FullSet.IsNull || pendingBinaryVars.HasValue)
                        {
                            break;
                        }
                        ...
                        // --- inlined CreateMultiValued ---
                        {
                            ...
                            if (numVars < numBinaryVars)
                            {
                                throw new InvalidOperationException("num_vars (1st field of .mv) must exceed num_binary_vars (2nd field of .mv)");
                            }

                            if (partSize.Length != numVars)
                            {
                                throw new InvalidOperationException("part_size length mismatch");
                            }
                    ...
                    case "type":
                        pla.PlaType = reader.NextToken() switch
                        {
                            ...
                            "dr" => PlaData.DrType,
                            "fdr" => PlaData.FdrType,
                            "eqn" or "eqntott" => PlaData.EqntottType,
                    ...
                    case "ilb":
                        if (cube.FullSet.IsNull)
                        {
                            throw new InvalidOperationException("PLA size must be declared before .ilb");
                        }

                        pla.Label ??= new string?[cube.Size];
                        for (int var = 0; var < cube.NumBinaryVars; var++)
                        {
                            string w = reader.NextToken()!;
                            int bi = cube.FirstPart[var];
                            pla.Label[bi + 1] = w;
                            pla.Label[bi] = $"{w}.bar";
                        }
                    ...
                    case "ob":
                        if (cube.FullSet.IsNull)
                        {
                            throw new InvalidOperationException("PLA size must be declared before .ob");
                        }
            ...
            else
            {
                ...
                if (pla.F == null)
                {
                    pla.F = BitVectorFamily.Create(10, cube.Size);
                    pla.D = BitVectorFamily.Create(10, cube.Size);
                    pla.R = BitVectorFamily.Create(10, cube.Size);
                }
                ...
                for (int var = 0; var < cube.NumBinaryVars; var++)
                {
                    ...
                    switch (token[ci++])
                    {
                        case '2':
                        case '-':
                            BitVectorOps.Insert(scf, var * 2 + 1);
                            BitVectorOps.Insert(scf, var * 2);
                            break;
                        case '0':
                            BitVectorOps.Insert(scf, var * 2);
                            break;
                ...
                for (int var = cube.NumBinaryVars; var < cube.NumVars - 1; var++)
                {
                    if (cube.PartSize[var] < 0)
                    {
                        ...
                        if (token is "-" or "ANY")
                        {
                            BitVectorOps.Or(scf, scf, cube.VarMask[var].AsSpan());
                        }
                        else if (token != "~")
                        {
                            bool found = false;
                            ...
                            {
                                if (pla.Label![ii] == null)
                                {
                                    pla.Label[ii] = token;
                                    BitVectorOps.Insert(scf, ii);
                                    found = true;
                                    break;
                                }

                                if (pla.Label[ii] == token)
                                {
                                    BitVectorOps.Insert(scf, ii);
                                    found = true;
                                    break;
                            ...
                            if (!found)
                            {
                                throw new InvalidOperationException($"declared size of variable {var} is too small");
                            }
                        }
                    }
                    else
                ...
                for (int ii = cube.FirstPart[cube.NumVars - 1]; ii <= cube.LastPart[cube.NumVars - 1]; ii++)
                {
                    ...
                    switch (token[oi++])
                    {
                        case '4':
                        case '1':
                            if ((pla.PlaType & PlaData.FType) != 0)
                            {
                                BitVectorOps.Insert(scf, ii);
                                savef = true;
                            }
                        ...
                        case '3':
                        case '0':
                            if ((pla.PlaType & PlaData.RType) != 0)
                            {
                                BitVectorOps.Insert(scr, ii);
                                saver = true;
                            }
                        ...
                        case '2':
                        case '-':
                            if ((pla.PlaType & PlaData.DType) != 0)
                            {
                                BitVectorOps.Insert(scd, ii);
                                saved = true;
                            }
                ...
                if (savef)
                {
                    pla.F = BitVectorFamily.Add(pla.F!, cube.Temp[0]);
                }

                if (saved)
                {
                    pla.D = BitVectorFamily.Add(pla.D!, cube.Temp[2]);
                }

                if (saver)
                {
                    pla.R = BitVectorFamily.Add(pla.R!, cube.Temp[1]);
                }
...
parseDone:
        ...
        else if (pla.PlaType == PlaData.FrType)
        {
            ...
            var mbdA = BitVectorFamily.Join(pla.F!, pla.R!);
            ...
            if (mbdLen == 0)
            {
                mbdResult = BitVectorFamily.FromSortedArray(mbdA1, 0, mbdA.SfSize);
            }
            else
            {
                ...
                {
                    if (mbdCompare(mbdA1[mbdii], mbdA1[mbdj]) == 0)
                    {
                        BitVectorOps.Or(mbdA1[mbdii].AsSpan(), mbdA1[mbdii].AsSpan(), mbdA1[mbdj].AsSpan());
                    }
                    else
                    {
    ...
    private sealed class LineReader(TextReader fp)
    {
        ...
        public string? NextToken()
        {
            while (true)
            {
                ...
                {
                    while (_pos < _currentLine.Length && char.IsWhiteSpace(_currentLine[_pos]))
                    {
                        _pos++;
                    }
        ...
        public void ResetToLine(string line)
        {
            _currentLine = line;
            _pos = 0;
        }
