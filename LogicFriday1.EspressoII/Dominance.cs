...
public static class Dominance
{
    public static int ApplyDominance(SparseMatrix A, bool isRow)
    {
        ...
        {
            if (!primary.TryGetValue(keyBuf[ki], out var entry))
            {
                continue;
            }
            ...
            {
                var s = secondary[r]!;
                if (s.Refs.Count < least.Refs.Count)
                {
                    least = s;
                }
            }
    ...
    internal static bool Visit(
        SparseMatrix A,
        SparseEntry entry,
        HashSet<int> visitedSelf,
        HashSet<int> visitedOther,
        SparseDict selfDict,
        SparseDict otherDict)
    {
        if (!visitedSelf.Add(entry.Key))
        {
            return false;
        }

        if (visitedSelf.Count == selfDict.Count)
        {
            return true;
        }
        ...
        foreach (int r in entry.Refs)
        {
            if (Visit(A, otherDict[r]!, visitedOther, visitedSelf, otherDict, selfDict))
            {
                return true;
            }
        }
