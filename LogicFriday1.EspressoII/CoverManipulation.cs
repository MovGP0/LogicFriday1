...
public static class CoverManipulation
{
    public static void CalculateCost(CubeData cube, BitVectorFamily F, out CoverCost cost)
    {
        ...
        {
            if (BitVectorOps.HasFlag(F.GetSet(si), CubeFlags.Prime))
            {
                cost.Primes++;
            }
        }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                // Inline HaveCommonActive
                {
                    ...
                    if (hcLast != -1)
                    {
                        ...
                        {
                            x = sp[w] | cofSpan[w];
                            y = seedSpan[w] | cofSpan[w];
                            if ((~(x & (x >> 1)) & ~(y & (y >> 1)) & BitVectorOps.Disjoint) != 0)
                            {
                                goto haveCommon;
                            }
                        }
                    ...
                    for (int var2 = cube.NumBinaryVars; var2 < cube.NumVars; var2++)
                    {
                        ...
                        for (int w = cube.FirstWordOf(var2); w <= mvLast; w++)
                        {
                            if ((sm[w] & ~sp[w] & ~cofSpan[w]) != 0)
                            {
                                ...
                                {
                                    if ((sm[w2] & ~seedSpan[w2] & ~cofSpan[w2]) != 0)
                                    {
                                        goto haveCommon;
                                    }
                                }
...
haveCommon:
        ...
        {
            A = new(BitVector.Null, [], 0);
            B = new(BitVector.Null, [], 0);
        }
    ...
    private static BitVectorFamily UnravelRange(CubeData cube, BitVectorFamily B, int start, int end)
    {
        ...
        {
            BitVectorOps.Or(sbSpan, sbSpan, cube.VarMask![var].AsSpan());
        }

        for (int var = end + 1; var < cube.NumVars; var++)
        {
            BitVectorOps.Or(sbSpan, sbSpan, cube.VarMask![var].AsSpan());
        }
        ...
        if (totalSize == B.Count)
        {
            return B;  // No expansion needed
        }
        ...
        for (int si = 0; si < B.Count; si++)
        {
            ...
            for (int var = start; var <= end; var++)
            {
                ...
                if ((size = BitVectorOps.IntersectionCount(c, vm)) < 2)
                {
                    BitVectorOps.Or(baseSpan, baseSpan, vm);
                }
                else
                {
                    expansion *= size;
            ...
            {
                BitVectorOps.Copy(B1.GetSpan(pi), baseSpan);
            }
            ...
            {
                ReadOnlySpan<uint> vm = cube.VarMask![var].AsSpan();
                if ((size = BitVectorOps.IntersectionCount(c, vm)) <= 1)
                {
                    continue;
                }
                ...
                for (int i = cube.FirstPart![var]; i <= cube.LastPart![var]; i++)
                {
                    ...
                    for (int j = n; j < expansion; j += skip)
                        ...
                        {
                            BitVectorOps.Insert(B1.GetSpan(j + k + offset), i);
                        }
