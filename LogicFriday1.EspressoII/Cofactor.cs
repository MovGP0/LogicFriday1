...
public static class Cofactor
{
    ...
    public static void BuildSplitCubes(CubeData cube, CubeList T, int best, Span<uint> cleft, Span<uint> cright)
    {
        ...
        for (int i = cube.FirstPart[best]; i <= lastbit; i++)
            if (!BitVectorOps.Contains(cof, i)) halfbit++;
        halfbit /= 2;
        int j = cube.FirstPart[best];
        for (; halfbit > 0; j++)
            if (!BitVectorOps.Contains(cof, j)) { halfbit--; BitVectorOps.Insert(cleft, j); }
        for (; j <= lastbit; j++)
            if (!BitVectorOps.Contains(cof, j)) BitVectorOps.Insert(cright, j);
    }
