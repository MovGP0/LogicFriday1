...
public class SparseMatrix(int rowCap = 16, int colCap = 16)
{
    ...
    public static SparseMatrix Clone(SparseMatrix A)
    {
        ...
        {
            B.Rows.Set(e.Key, new SparseEntry { Key = e.Key, Refs = new SortedIntArray(e.Refs) });
        }

        foreach (var e in A.Cols)
        {
            B.Cols.Set(e.Key, new SparseEntry { Key = e.Key, Refs = new SortedIntArray(e.Refs) });
        }
    ...
    {
        if (!A.Rows.TryGetValue(row, out var prow))
        {
            prow = new SparseEntry { Key = row };
            A.Rows.Set(row, prow);
            if (row > A.LastRowNum)
            {
                A.LastRowNum = row;
    ...
    {
        if (!primary.TryGetValue(i, out var entry))
        {
            return;
        }
