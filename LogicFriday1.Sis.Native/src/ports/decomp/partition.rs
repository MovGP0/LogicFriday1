...
pub struct DecompositionBlockPartition {
...
impl DecompositionBlockPartition {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DecompositionComponentVisit {
    pub rows: BTreeSet<usize>,
    pub cols: BTreeSet<usize>,
}

impl DecompositionComponentVisit {
...
pub fn dec_block_partition(matrix: &SparseMatrix) -> Option<DecompositionBlockPartition> {
    block_partition(matrix)
}

pub fn block_partition(matrix: &SparseMatrix) -> Option<DecompositionBlockPartition> {
    ...
    for row in matrix.rows() {
        if visit.rows.contains(&row.index()) {
            copy_row(&mut left, matrix, row.index());
        } else {
            copy_row(&mut right, matrix, row.index());
        }
    }

    Some(DecompositionBlockPartition {
        left,
        right,
...
pub fn visit_from_first_row(matrix: &SparseMatrix) -> Option<DecompositionComponentVisit> {
    let first_row = matrix.rows().next()?.index();

    Some(visit_component_from_row(matrix, first_row))
}

pub fn visit_component_from_row(
    matrix: &SparseMatrix,
    start_row: usize,
) -> DecompositionComponentVisit {
    ...
    DecompositionComponentVisit { rows, cols }
}

pub fn copy_row(dest: &mut SparseMatrix, source: &SparseMatrix, row_num: usize) {
...
mod tests {
    ...
    fn empty_and_connected_matrices_do_not_partition() {
        assert_eq!(dec_block_partition(&SparseMatrix::new()), None);

        let matrix = matrix_from_pairs(&[(0, 0), (1, 0), (1, 2), (3, 2)]);

        assert_eq!(dec_block_partition(&matrix), None);
    }
    ...
    fn partitions_rows_by_component_reached_from_first_row() {
        ...
        let partition = dec_block_partition(&matrix).unwrap();

        assert!(partition.is_nontrivial());
    ...
    #[test]
    fn starts_from_first_sparse_row_in_sorted_order() {
        let matrix = matrix_from_pairs(&[(5, 8), (2, 8), (0, 1)]);

        let partition = dec_block_partition(&matrix).unwrap();

        assert_eq!(partition.visited_rows, set(&[0]));
        assert_eq!(partition.visited_cols, set(&[1]));
        assert_eq!(partition.left.write_pairs(), "0 1\n");
        assert_eq!(partition.right.write_pairs(), "2 8\n5 8\n");
    }

    #[test]
    fn component_visit_walks_alternating_row_and_column_edges() {
