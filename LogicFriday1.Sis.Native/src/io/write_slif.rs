...
use crate::network::network_util::{
...
fn render_expression(
    expression: &crate::network::network_util::BoolExpr,
    network: &Network,
    use_short_names: bool,
) -> WriteSlifResult<String> {
    match expression {
        crate::network::network_util::BoolExpr::Constant(value) => {
            Ok(if *value { "1" } else { "0" }.to_string())
        }
        crate::network::network_util::BoolExpr::Literal { node, phase } => {
        ...
        crate::network::network_util::BoolExpr::Not(inner) => Ok(format!(
            "({})'",
            render_expression(inner, network, use_short_names)?
        )),
        crate::network::network_util::BoolExpr::And(parts) => {
            render_expression_list(parts, " ", network, use_short_names)
        }
        crate::network::network_util::BoolExpr::Or(parts) => {
            render_expression_list(parts, " + ", network, use_short_names)
        }
    }
}
...
fn render_expression_list(
    parts: &[crate::network::network_util::BoolExpr],
    separator: &str,
    network: &Network,
    use_short_names: bool,
) -> WriteSlifResult<String> {
...
mod tests {
    use super::*;
    use crate::network::network_util::{BoolExpr, Cube, SopCover};
